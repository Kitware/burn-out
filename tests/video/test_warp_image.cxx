            ...
            vxl_byte r, vxl_byte g = 0, vxl_byte b = 0 )
{
  ...
  if( ! good )
  {
    vcl_cout << "At ("<<i<<","<<j<<"), expecting (got)";
    if( img.nplanes() >= 1 )
    {
      vcl_cout << " " << int(r) << "("<<int(img(i,j,0))<<")";
    }
    if( img.nplanes() >= 2 )
    {
      vcl_cout << " " << int(g) << "("<<int(img(i,j,1))<<")";
    }
    if( img.nplanes() >= 3 )
    {
      vcl_cout << " " << int(b) << "("<<int(img(i,j,2))<<")";
    }
    vcl_cout << "\n";
  }
...
test_simple_warp_image()
{
  ...
  {
    vcl_cout << "Unmapped fill parameter\n";
    h.set_translation( 4, 7 );
    vil_image_view<vxl_byte> dest( 32, 32, 1, 3 );
    dest.fill( 100 );
