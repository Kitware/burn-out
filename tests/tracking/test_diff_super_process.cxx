...
void setup_and_test_disabled_sp( bool disabled, bool run_async )
{
  ...
  catch( unchecked_return_value const& )
  {
  ...
  dsp.set_src_to_ref_homography( image_to_image_homography() );
  dsp.set_src_to_wld_homography( image_to_plane_homography() );
  dsp.set_wld_to_src_homography( plane_to_image_homography() );
  dsp.set_wld_to_utm_homography( plane_to_utm_homography() );
  dsp.set_world_units_per_pixel( 1.0 );
  dsp.set_input_timestamp_vector( vcl_vector< timestamp >( 1, ts ) );
  dsp.set_input_ref_to_wld_homography( image_to_plane_homography() );
