...
#include "variable_image_dilate.h"
                            ...
                            vil_image_view<PixType>& dst )
{
  ...
  // Perform dilation
  variable_image_dilate( src, elem_ids, se_vector, se_offsets, largest, dst );
}



// Core [unsafe] implementation for performing a variable dilation.
template< typename PixType, typename IDType >
void variable_image_dilate( const vil_image_view<PixType>& src,
                            const vil_image_view<IDType>& elem_ids,
                            const structuring_element_vector& se_vector,
                            ...
                            vil_image_view<PixType>& dst )
{
  ...
  for (int i=0;i<ilo;++i)
  {
    ...
    {
      dst(i,j,0)=vil_greyscale_dilate(src,0,se_vector[elem_ids(i,j)],i,j);
    }
  }
  ...
  for (int j=jlo;j<=jhi;++j)
  {
    ...
    for (int i=ilo;i<=ihi;++i,src_p+=s_istep,dst_p+=d_istep,esm_p+=m_istep)
    {
      ...
      *dst_p=vil_greyscale_dilate(src_p,&offsets[0],offsets.size());
    }
  }
}
                            ...
                            vil_image_view<bool>& dst )
{
  ...
  const vil_border<vil_image_view<bool> > border = vil_border_create_constant(src, false);
  ...
  for (int i=0;i<ilo;++i)
  {
    for (unsigned int j=0;j<nj;++j)
    {
      dst(i,j,0)=vil_binary_dilate(border_accessor,0,se_vector[elem_ids(i,j)],i,j);
    }
  }
  // Deal with right edge
  for (unsigned int i=ihi+1;i<ni;++i)
  ...
  for (int j=jlo;j<=jhi;++j)
  {
    ...
    {
      const std::vector< std::ptrdiff_t >& offsets = offset_vec[*esm_p];
      *dst_p=vil_binary_dilate(src_p,&offsets[0],offsets.size());
    }
  }
}
