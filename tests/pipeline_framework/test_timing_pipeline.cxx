...
test_parallel_processes( unsigned num_procs )
{
  ...
  std::vector< process_smart_pointer< busy_loop > > busy_loops;
  std::vector< process_smart_pointer< collect_value > > collectors;

  p.add( nums );

  for( unsigned i = 0; i < num_procs; ++i )
  {
    process_smart_pointer< busy_loop > busy( new busy_loop( std::string("busy_loop_") + char('A' + i), NUM_BUSY_LOOPS ) );
    process_smart_pointer< collect_value > collect( new collect_value( std::string("collect_") + char('A' + i) ) );
...
test_sequential_processes( unsigned num_procs )
{
  ...
  {
    busy = new busy_loop( std::string("busy_loop_") + char('A' + i), NUM_BUSY_LOOPS  );
...
test_parallel_nonuniform_processes( unsigned num_procs )
{
  ...
  for( unsigned i = 0; i < num_procs; ++i )
  {
    ...
    process_smart_pointer< busy_loop > busy( new busy_loop( std::string("busy_loop_") + char('A' + i),  loop_multiple * NUM_BUSY_LOOPS  ) );
    process_smart_pointer< collect_value > collect( new collect_value( std::string("collect_") + char('A' + i) ) );
...
void busy_job()
{
  ...
  for(unsigned int i=0; i<NUM_BUSY_LOOPS*10; ++i)
  {
    ...
    v += i + static_cast<unsigned>(std::sqrt(v2*v2));
  }
}
...
int test_timing_pipeline( int /*argc*/, char* /*argv*/[] )
{
  ...
  unsigned num_procs = boost::thread::hardware_concurrency();
  std::cout << "detected "<< num_procs << " logical CPUs" << std::endl;
  if ( num_procs < 2 )
  {
    std::cerr << "This test requires at least 2 CPU cores" << std::endl;
    std::abort();
  }
  num_procs = real_hardware_concurrency();
  std::cout << "using "<< num_procs << " concurrent threads" << std::endl;
