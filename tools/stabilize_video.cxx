...
int main( int argc, char** argv )
{
  ...
  vidtk::process_smart_pointer< vidtk::generic_frame_process<vxl_byte> > frame_src( new vidtk::generic_frame_process<vxl_byte>( "src" ));
  p.add( frame_src );

  vidtk::process_smart_pointer< vidtk::stabilization_super_process<vxl_byte> > stab_sp(new vidtk::stabilization_super_process<vxl_byte>( "stab_sp" ));
  p.add( stab_sp );
  p.connect( frame_src->image_port(), stab_sp->set_source_image_port() );
  p.connect( frame_src->timestamp_port(), stab_sp->set_source_timestamp_port() );

  vidtk::process_smart_pointer< vidtk::image_list_writer_process<vxl_byte> > gui_window_writer(new vidtk::image_list_writer_process<vxl_byte>( "gui_writer" ));
  p.add( gui_window_writer );
  p.connect( stab_sp->source_timestamp_port(), gui_window_writer->set_timestamp_port() );
  ...
  vgui::adapt( shell_tab, stab_sp->source_image().ni()*mult, stab_sp->source_image().nj(), main_menu );
  ...
  vidtk::process::step_status s;

  do
  {
    vcl_cout << "Done processing time = ";
    if( stab_sp->source_timestamp().has_time() )
    {
      vcl_cout << vul_sprintf("%.3f",stab_sp->source_timestamp().time_in_secs());
    }
    if(stab_sp->source_timestamp().has_frame_number() )
    {
      vcl_cout << " (frame="<< stab_sp->source_timestamp().frame_number() << ")";
    }
    vcl_cout << "\n";

    img_tab->set_image_view( stab_sp->source_image() );
...

    {
      ...
      track_container const& trks = stab_sp->active_tracks();
      for( iter_type i = trks.begin(); i != trks.end(); ++i )
      {
        ...
        //else
        //{
        //  bad_track_drawer.draw_world2d( **i );
        //}
      }
    ...
    if( show_stab() )
    {
      vil_image_view<vxl_byte> const& curr = stab_sp->source_image();
      vcl_cout << "H=\n"<<stab_sp->src_to_ref_homography()<<"\n";
      if( ! dest )
      {
        unsigned ni = curr.ni();
        unsigned nj = curr.nj();
        compute_output_size( ni, nj, dest_i0, dest_j0, stab_sp->src_to_ref_homography().get_transform() );
        dest = vil_image_view<vxl_byte>( ni+2000, nj+2000, 1, curr.nplanes() );
      }
      vidtk::warp_image( curr, dest, stab_sp->ref_to_src_homography().get_transform(), dest_i0, dest_j0 );
...
//       typedef vcl_list< vidtk::track_sptr > track_container;
//       typedef track_container::const_iterator iter_type;
//       track_container const& trks = stab_sp->active_tracks();
//       for( iter_type i = trks.begin(); i != trks.end(); ++i )
//       {
//         vnl_vector_fixed<double,3> init_pt = (*i)->history()[0]->loc_;
//         vnl_vector_fixed<double,3> curr_pt = (*i)->last_state()->loc_;
...
//     }
    ...
    if( stab_sp->get_output_shot_break_flags().is_shot_end() )
    {
      vcl_cout << "RESET" << vcl_endl;
      p.reset();
      s = vidtk::process::SKIP;
    }

  } while( ! vgui::quit_was_called()
           && ( s != vidtk::process::FAILURE ) );
...
//   vcl_vector< vidtk::track_sptr > active_vector;
//   const vcl_list< vidtk::track_sptr>& active_list = tracker.active_tracks();
//
//   for (vcl_list< vidtk::track_sptr>::const_iterator t = active_list.begin();
//        t != active_list.end();
//        ++t)
//   {
//     active_vector.push_back( *t );
//   }
