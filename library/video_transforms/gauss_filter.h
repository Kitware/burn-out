...
#include <video_transforms/convolve_1d.h>

namespace vidtk
{

inline vxl_int_64 gen_int_gauss_filter(double sd, std::vector<unsigned> &filter)
{
  ...
  */
  ...
  double p = static_cast<double>(num);
  double q = static_cast<double>(den);
  ...
  vxl_int_64 G_0 = static_cast<vxl_int_64>(std::pow(p,xm_sq));

  //6)
  std::size_t center = filter.size()/2; // or just past center if even length
  std::vector< vxl_int_64 > tmp_filter( filter.size() );
  ...
  for (unsigned i=1 ; i<=center; ++i)
  {
    ...
    int xj_s = i*i;
    vxl_int_64 p_xj = static_cast<vxl_int_64>(std::pow( p, (xm_sq - xj_s) ));
    vxl_int_64 q_xj = static_cast<vxl_int_64>(std::pow( q, xj_s ));
  ...
  if ( tmp_filter[0] > std::numeric_limits<unsigned>::max() )
  {
    return -1;
  }
  filter[center] = static_cast<unsigned>(tmp_filter[0]);
  ...
  for (unsigned i=1 ; i<=center; ++i)
  {
    ...
    vxl_int_64 G_xj = vnl_math::rnd_halfintup(G_0/( std::pow( p_q_ratio, xj_s) ));

    filter[center+i] = filter[center-i] = static_cast<unsigned>(G_xj);
  }

  vxl_int_64 kernel_sum = std::accumulate(filter.begin(), filter.end(), 0);
...
inline bool gauss_filter_2d_int(const vil_image_view<srcT>& src_im,
                                vil_image_view<destT>& dest_im,
                                double sd, unsigned half_width)
{
  // Generate filter
  std::vector< unsigned > filter(2 * half_width + 1);
  vxl_int_64 kernel_sum = gen_int_gauss_filter(sd,filter);
  ...
  vxl_int_64 original_sum = std::numeric_limits<srcT>::max() * kernel_sum;

  if ( original_sum >= std::numeric_limits<unsigned>::max() )
  {
    return false;
  }
  //since we're only normalizing at the end, make sure our biggest value
  //on the second convolve operation will fit.
  if ( original_sum*kernel_sum >= std::numeric_limits<vxl_int_64>::max() )
  {
    return false;
  }
  ...
  return true;
}//inline bool gauss_filter_2d_int
