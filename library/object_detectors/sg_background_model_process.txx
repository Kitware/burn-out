...
#define VIDTK_DEFAULT_LOGGER __vidtk_logger_auto_sg_background_model_process_txx__
VIDTK_LOGGER("sg_background_model_process_txx");
...
::sg_background_model_process( std::string const& _name )
  : fg_image_process<PixType>( _name, "sg_background_model_process" ),
    param_( new sg_gm_pixel_model_params ),
    inp_frame_( NULL )
{
  config_.add_parameter( "sigma_threshold", "2.5", "UNDOCUMENTED" );
  config_.add_parameter( "initial_sigma", "10", "UNDOCUMENTED" );
  config_.add_parameter( "initial_weight", "0.01", "UNDOCUMENTED" );
  config_.add_parameter( "alpha_weights", "0.01", "UNDOCUMENTED" );
  config_.add_parameter( "alpha_means", "0.01", "UNDOCUMENTED" );
  config_.add_parameter( "alpha_variance", "0.01", "UNDOCUMENTED" );
  config_.add_parameter( "learning_alpha_weights", "0.01", "UNDOCUMENTED" );
  config_.add_parameter( "learning_alpha_means", "0.01", "UNDOCUMENTED" );
  config_.add_parameter( "learning_alpha_variance", "0.01", "UNDOCUMENTED" );
  config_.add_parameter( "rho_mean", "-1", "UNDOCUMENTED" );
  config_.add_parameter( "rho_variance", "-1", "UNDOCUMENTED" );
  config_.add_parameter( "background_threshold", "0.9", "UNDOCUMENTED" );
  config_.add_parameter( "order_by_weight_only", "false", "UNDOCUMENTED" );
  config_.add_parameter( "num_mixture_components", "5", "UNDOCUMENTED" );
  config_.add_parameter( "min_samples_for_detection", "0", "UNDOCUMENTED" );
  config_.add_parameter( "no_update_threshold", "-1.0", "UNDOCUMENTED" );
  config_.add_parameter( "stabilization_padding", "100", "UNDOCUMENTED" );
  config_.add_parameter( "take_first_matched", "true", "UNDOCUMENTED");
}
...
::bg_image() const
{
  ...
  std::vector<sg_gm_pixel_model::float_type> value( nplanes );
...
::initialize_array( unsigned ni, unsigned nj, unsigned nplanes )
{
  ...
  prob_image_.set_size(ni, nj);
  prob_image_.fill(0.0f);
}
...
::warp_array()
{
  ...
  for( unsigned j = 0; j < new_models.rows(); j++ )
  {
    for( unsigned i = 0; i < new_models.cols(); i++ )
    {
      ...
      if( old_j < 0 || old_i < 0 ||
          old_j >= static_cast<int>(models_.rows()) ||
          old_i >= static_cast<int>(models_.cols()) )
      {
...
::step()
{
  ...
  std::vector<float_type> pix_value( nplanes );
  ...
  for( unsigned j = 0; j < img.nj(); ++j )
  {
    for( unsigned i = 0; i < img.ni(); ++i )
    {
      ...
      if( new_x >= models_.cols() ||
          new_y >= models_.rows() )
      {
        LOG_INFO( "Warped image out of array bounds. Setting new reference image.");
      ...
      float confidence;
      fgnd_mask_(i, j) = models_(new_y, new_x)->update(&pix_value[0], confidence);
      prob_image_(i, j) = confidence;
    }
  }
