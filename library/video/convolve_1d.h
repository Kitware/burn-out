...
inline void convolve_edge_1d(const srcT* src, unsigned n, vcl_ptrdiff_t s_step,
                                 destT* dest, vcl_ptrdiff_t d_step,
                                 const unsigned* kernel,
                                 vcl_ptrdiff_t k_lo, vcl_ptrdiff_t k_hi,
                                 vcl_ptrdiff_t kstep, accumT ac, float norm)
{
...
// Note* We are multiplying the inverse rather than dividing for performance reasons.
template <class srcT, class destT, class accumT>
inline void convolve_1d(const srcT* src0, unsigned nx, vcl_ptrdiff_t s_step,
                            destT* dest0, vcl_ptrdiff_t d_step,
                            const unsigned* kernel,
                            vcl_ptrdiff_t k_lo, vcl_ptrdiff_t k_hi,
                            accumT ac, float norm)
{
  assert(k_hi - k_lo < static_cast<unsigned>(nx));
                        ...
                        accumT ac, float norm )
{
  ...
  assert(k_hi - k_lo +1 <= (unsigned) n_i);
