...
public:
  ...
  paired_buffer_process( vcl_string const& );
  ...
  void set_updated_items( vcl_vector< pair_t > const&  );
  VIDTK_INPUT_PORT( set_updated_items, vcl_vector< pair_t > const& );

  paired_buff_t const& buffer() const;
  VIDTK_OUTPUT_PORT( paired_buff_t const&, buffer );
