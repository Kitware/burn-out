...
def TrainAdaboostClassifer( prefix ) :
  args=["train_adaboost_classifer", "--input-file", "driver.txt", "--output-prefix", prefix ]
...
def CreateTrainingSet(config_file, base_directory, experiment_name, output_file, make_check_video ) :
...
def main() :
  ...
  argParser.add_option("-e", dest="experiments",
                       help="(required) File containing the set of regions to train on")
  ...
  while 1 :
    ...
    aoi = exp_elements[2]
    CreateTrainingSet( options.config_file, base_director, experiment_name,
