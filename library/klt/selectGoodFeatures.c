  ...
  KLT_BOOL overwriteAllFeatures)
{
  ...
  memset(featuremap, 0, (unsigned int)(ncols*nrows));
...
void _KLTSelectGoodFeaturesRaw(
    ...
    selectionMode mode)
{
  ...
  pointlist = (int *) malloc(img->ncols * img->nrows * 3 * sizeof(int));
     ...
     of the two eigenvalues of the Z matrix */
  {
    ...
    ptr = pointlist;
    for (y = bordery ; y < img->nrows - bordery ; y += tc->nSkippedPixels + 1)
      for (x = borderx ; x < img->ncols - borderx ; x += tc->nSkippedPixels + 1)  {

        /* Sum the gradients in the surrounding window */
        gxx = 0;  gxy = 0;  gyy = 0;
        for (yy = y-window_hh ; yy <= y+window_hh ; yy++) {
          float* tgx = gradx->data + img->ncols*yy;
          float* tgy = grady->data + img->ncols*yy;
  ...
  selectionMode mode)
{
  ...
  _KLTSelectGoodFeaturesRaw(tc, floatimg, gradx, grady, featurelist, mode);
  ...
  KLT_FeatureList fl)
{
  if (KLT_verbose >= 1)  {
    fprintf(stderr,  "(KLT) Selecting the %d best features "
            "from a %d by %d image...  ", fl->nFeatures, img->ncols, img->nrows);
    fflush(stderr);
  }

  _KLTSelectGoodFeaturesRaw(tc, img, gradx, grady, 
                            fl, SELECTING_ALL);
  ...
  KLT_FeatureList fl)
{
  ...
  if (KLT_verbose >= 1)  {
    fprintf(stderr,  "\t%d features replaced.\n",
...
void KLTReplaceLostFeaturesRaw(
  KLT_TrackingContext tc,
  _KLT_FloatImage img, 
  _KLT_FloatImage grady, 
  _KLT_FloatImage gradx,
  KLT_FeatureList fl)
{
  ...
  if (KLT_verbose >= 1)  {
    fprintf(stderr,  "(KLT) Attempting to replace %d features "
            "in a %d by %d image...  \n", nLostFeatures, img->ncols, img->nrows);
    fflush(stderr);
  }
