...
// ----------------------------------------------------------------
/** Reads and Writes a set of data types.
 *
...
{
public:
  typedef vcl_vector < base_reader_writer * > rw_vector_t;
  typedef rw_vector_t::iterator rw_iterator_t;


  /** Input file / object list mismatch.
   *
  ...
  {
  public:
    /** Unrecognized input found. This method is called when a line is
     * discovered in the input stream that is not claimed by a reader
     * object. A policy handler could look for a specific tag value
     * and perform some operation. This will allow a tag to initiate
     * an execution of something.
     *
    ...
    /** Missing input detected. This method is called when there are
     * reader objects that have not found input in the input stream.
     *
     * One example implementation could look for default values for
     * unsatisfied objects from an alternate source.
     *
     * @param[in] list of objects that have not received input.
     *
     * @return Number of objects that still do not have input. A zero
     * indicates that all objects have been satisfied.
     */
    virtual int missing_input (rw_vector_t const& objs) = 0;
  };
...
protected:
  bool skip_line(vcl_istream & str) const;
  int skip_comment(vcl_istream & str) const;
  int find_group_start(vcl_istream & str, vcl_string & line);
