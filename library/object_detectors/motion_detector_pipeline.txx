...
#define VIDTK_DEFAULT_LOGGER __motion_detector_pipeline_txx__
VIDTK_LOGGER ("motion_detector_pipeline");

template< class PixType >
motion_detector_pipeline< PixType >
::motion_detector_pipeline( std::string const& n )
    ...
    proc_set_proj_obj_type( NULL )
{
  ...
  // Create separate functors to set the source information
  this->m_obj_type = boost::make_shared< set_type_functor >( vidtk::image_object::MOTION, this->name() );
  ...
  proc_set_proj_obj_type = new set_object_type_process( "set_motion_type_proj", this->m_proj_obj_type );
}


template< class PixType >
motion_detector_pipeline< PixType >
::~motion_detector_pipeline()
{
}
...
// ----------------------------------------------------------------
template < class PixType >
bool motion_detector_pipeline< PixType >
  ::set_params( config_block const& blk )
{
  ...
  try
  {
    ...
    { // Reflect cropping setting into appropriate uncrop processes in children
      ...
      dependency_config.add_parameter( proc_conn_comp_sp->name() + ":fg_image_uncrop:horizontal_padding",
                                       crop_ul_awk[0],
                                       "Forced override" );
      dependency_config.add_parameter( proc_conn_comp_sp->name() + ":fg_image_uncrop:vertical_padding",
                                       crop_ul_awk[1],
                                       "Forced override" );
      dependency_config.add_parameter( proc_conn_comp_sp->name() + ":fg_image_uncrop:disabled",
                                       blk.get<std::string>( proc_conn_comp_sp->name() + ":fg_image_writer:disabled" ),
                                       "Forced override" );
    }
...
::setup_pipeline( Pipeline * p )
{
  ...
  connect_ports( proc_conn_comp_sp, output_objects,        proc_set_obj_type, input );
  connect_ports( proc_conn_comp_sp, projected_objects,     proc_set_proj_obj_type, input );
