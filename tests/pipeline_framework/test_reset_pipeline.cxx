  ...
  : public vidtk::process
{
  ...
  fail_gen( std::string const& _name,
            unsigned fail_value = 15,
            unsigned constant = 5 )
    : vidtk::process( _name, "fail_gen" ),
  ...
  : public vidtk::super_process
{
  ...
  recoverable_sp( std::string const& _name )
    : vidtk::super_process( _name, "recoverable_sp" ),
  ...
  bool fail_recover()
  {
    ...
    {
      std::cout << "reseting pipeline at "<<fail_gen2_->name()<<std::endl;
      return pipeline_->reset_downstream( fail_gen2_.ptr() );
    }
    else if( fail_gen1_->generated_failure_ )
    {
      std::cout << "reseting pipeline at "<<fail_gen1_->name()<<std::endl;
...
test_reset()
{
  ...
  unsigned num_itr = std::min(static_cast<unsigned>(collect->values_.size()), 18u);
