				  ...
				  float *Axy, float *Ayy)        /* used affine mapping */
{
  ...
  /* avoid "may be used uninitialized" warning with gcc -O3 */
  dx = 0;
  dy = 0;
					  ...
					  KLT_FeatureList featurelist)
{
	...
	int i;
	KLT_TrackingPyramidRec pyramid1;
	KLT_TrackingPyramidRec pyramid2;
	...
	if (tc->sequentialMode && tc->pyramid_last != NULL)  {
		pyramid1.pyramid = tc->pyramid_last;
		pyramid1.pyramid_gradx = tc->pyramid_last_gradx;
		pyramid1.pyramid_grady = tc->pyramid_last_grady;
		if (pyramid1.pyramid->ncols[0] != ncols || pyramid1.pyramid->nrows[0] != nrows)
			KLTError("(KLTTrackFeatures) Size of incoming image (%d by %d) "
			"is different from size of previous image (%d by %d)\n",
			ncols, nrows, pyramid1.pyramid->ncols[0], pyramid1.pyramid->nrows[0]);
		assert(pyramid1.pyramid_gradx != NULL);
		assert(pyramid1.pyramid_grady != NULL);
	} else  {
		...
		pyramid1.pyramid = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
		_KLTComputePyramid(floatimg1, pyramid1.pyramid, tc->pyramid_sigma_fact);
		pyramid1.pyramid_gradx = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
		pyramid1.pyramid_grady = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
		for (i = 0 ; i < tc->nPyramidLevels ; i++)
			_KLTComputeGradients(pyramid1.pyramid->img[i], tc->grad_sigma, 
			pyramid1.pyramid_gradx->img[i],
			pyramid1.pyramid_grady->img[i]);
	}
	...
	pyramid2.pyramid = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
	_KLTComputePyramid(floatimg2, pyramid2.pyramid, tc->pyramid_sigma_fact);
	pyramid2.pyramid_gradx = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
	pyramid2.pyramid_grady = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
	for (i = 0 ; i < tc->nPyramidLevels ; i++)
		_KLTComputeGradients(pyramid2.pyramid->img[i], tc->grad_sigma, 
		pyramid2.pyramid_gradx->img[i],
		pyramid2.pyramid_grady->img[i]);

	KLTTrackFeaturesPyramid(tc, &pyramid1, &pyramid2, ncols, nrows, featurelist);

	/* Free memory */
	_KLTFreeFloatImage(tmpimg);
	if (floatimg1_created)  _KLTFreeFloatImage(floatimg1);
	_KLTFreeFloatImage(floatimg2);
	_KLTFreePyramid(pyramid1.pyramid);
	_KLTFreePyramid(pyramid1.pyramid_gradx);
	_KLTFreePyramid(pyramid1.pyramid_grady);
}


/*********************************************************************
 * KLTTrackFeaturesPyramid
 *
 * Tracks feature points from one image pyramid to the next.
 */

void KLTTrackFeaturesPyramid(
					  KLT_TrackingContext tc,
					  KLT_TrackingPyramid pyramid1,
					  KLT_TrackingPyramid pyramid2,
					  int ncols,
					  int nrows,
					  KLT_FeatureList featurelist)
{
	int const p1Subsampling = pyramid1->pyramid->subsampling;
	int const p2Subsampling = pyramid2->pyramid->subsampling;

	float const subsampling = (float) p1Subsampling;

	int val;
	int indx, r;
	int i;

	const int p1nLevels = pyramid1->pyramid->nLevels;
	const int p2nLevels = pyramid1->pyramid->nLevels;

	const int nLevels = (p1nLevels < p2nLevels) ? p1nLevels : p2nLevels;

	float xloc, yloc, xlocout, ylocout;

	if (p1Subsampling != p2Subsampling) {
		printf("Error: The first pyramid (%d) and second pyramid (%d) subsamplings are not the same.", p1Subsampling, p2Subsampling);
		return;
	}

#ifndef NDEBUG
	if (subsampling != tc->subsampling) {
		printf("Warning: The pyramid (%f) and suggested (%d) subsamplings are not the same.", subsampling, tc->subsampling);
	}
#endif
...
#ifndef NDEBUG
	if (p1nLevels != p2nLevels) {
		printf("Warning: The first pyramid (%d) and second pyramid (%d) do not have the same number of levels (using the lesser).", p1nLevels, p2nLevels);
	}
#endif

#ifndef NDEBUG
	if (nLevels != tc->nPyramidLevels) {
		printf("Warning: The pyramid (%d) and suggested (%d) levels are not the same.", nLevels, tc->nPyramidLevels);
	}
#endif
	...
	if (tc->writeInternalImages)  {
		...
		for (i = 0 ; i < nLevels ; i++)  {
			...
			_KLTWriteFloatImageToPGM(pyramid1->pyramid->img[i], fname);
			sprintf(fname, "kltimg_tf_i%d_gx.pgm", i);
			_KLTWriteFloatImageToPGM(pyramid1->pyramid_gradx->img[i], fname);
			sprintf(fname, "kltimg_tf_i%d_gy.pgm", i);
			_KLTWriteFloatImageToPGM(pyramid1->pyramid_grady->img[i], fname);
			sprintf(fname, "kltimg_tf_j%d.pgm", i);
			_KLTWriteFloatImageToPGM(pyramid2->pyramid->img[i], fname);
			sprintf(fname, "kltimg_tf_j%d_gx.pgm", i);
			_KLTWriteFloatImageToPGM(pyramid2->pyramid_gradx->img[i], fname);
			sprintf(fname, "kltimg_tf_j%d_gy.pgm", i);
			_KLTWriteFloatImageToPGM(pyramid2->pyramid_grady->img[i], fname);
		}
#endif
	}
		...
		if (featurelist->feature[indx]->val >= 0)  {
			...
			for (r = nLevels - 1 ; r >= 0 ; r--)  {
				...
				val = _trackFeature(xloc, yloc, 
					&xlocout, &ylocout,
					pyramid1->pyramid->img[r], 
					pyramid1->pyramid_gradx->img[r], pyramid1->pyramid_grady->img[r], 
					pyramid2->pyramid->img[r], 
					pyramid2->pyramid_gradx->img[r], pyramid2->pyramid_grady->img[r],
			...
			if (val == KLT_OOB) {
				...
				if (tc->affineConsistencyCheck >= 0 && val == KLT_TRACKED)  { /*for affine mapping*/
					...
					if(!featurelist->feature[indx]->aff_img){
						...
						_am_getSubFloatImage(pyramid1->pyramid->img[0],xloc,yloc,featurelist->feature[indx]->aff_img);
						_am_getSubFloatImage(pyramid1->pyramid_gradx->img[0],xloc,yloc,featurelist->feature[indx]->aff_img_gradx);
						_am_getSubFloatImage(pyramid1->pyramid_grady->img[0],xloc,yloc,featurelist->feature[indx]->aff_img_grady);
						...
						val = _am_trackFeatureAffine(featurelist->feature[indx]->aff_x, featurelist->feature[indx]->aff_y,
							...
							pyramid2->pyramid->img[0], 
							pyramid2->pyramid_gradx->img[0], pyramid2->pyramid_grady->img[0],
	...
	if (tc->sequentialMode)  {
		tc->pyramid_last = pyramid2->pyramid;
		tc->pyramid_last_gradx = pyramid2->pyramid_gradx;
		tc->pyramid_last_grady = pyramid2->pyramid_grady;
	} else  {
		_KLTFreePyramid(pyramid2->pyramid);
		_KLTFreePyramid(pyramid2->pyramid_gradx);
		_KLTFreePyramid(pyramid2->pyramid_grady);
	}

	if (KLT_verbose >= 1)  {
		fprintf(stderr,  "\t%d features successfully tracked.\n",
