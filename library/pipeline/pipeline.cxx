...
::~pipeline()
{
  typedef vcl_map<void*,pipeline_node*>::iterator node_itr;
...
void
pipeline
::add_execution_dependency( void* source_id, void* sink_id )
{
...
void
pipeline
::add_optional_execution_dependency( void* source_id, void* sink_id )
{
  pipeline_node* source_node = get_node_with_id( source_id );
  pipeline_node* sink_node = get_node_with_id( sink_id );
...
void
pipeline
::do_connect( void* source_id, void* sink_id, pipeline_edge* edge)
{
  pipeline_node* source_node = get_node_with_id( source_id );
  pipeline_node* sink_node = get_node_with_id( sink_id );
...
::initialize()
{
  ...
  for( itr it = execution_order_.begin(); it != execution_order_.end(); ++it ) {
    if( ! (*it)->initialize() ) {
      log_warning( "Failed to initialize node " << (*it)->name() << "\n" );
      return false;
    }
  }
...
::reset()
{
  ...
  for( itr it = execution_order_.begin(); it != execution_order_.end(); ++it ) {
    if( ! (*it)->reset() ) {
      log_warning( "Failed to reset node " << (*it)->name() << "\n" );
      return false;
    }
  }
...
bool
pipeline
::reset_downstream(void* id)
{
  // find all downstream nodes
  vcl_vector< pipeline_node* > downstream;
  downstream = downstream_nodes( get_node_with_id( id ) );

  // reset all the downstream nodes
  typedef vcl_vector<pipeline_node*>::iterator node_itr;
  for( node_itr it = downstream.begin(); it != downstream.end(); ++it )
  {
    if( ! (*it)->reset() )
    {
      log_error( "Failed to reset node " << (*it)->name() << "\n" );
      return false;
    }
  }
...
bool
pipeline
::set_params_downstream( config_block const& all_params, void* id)
{
  // find all downstream nodes
  vcl_vector< pipeline_node* > downstream;
  downstream = downstream_nodes( get_node_with_id( id ) );
  ...
  for( node_itr it = downstream.begin(); it != downstream.end(); ++it )
  {
    vcl_cout << "Set parameters for " << (*it)->name() << "\n";
...
void
pipeline::
set_output_node( void* node_id, bool value )
{
...
void
pipeline
::output_graphviz_file( vcl_string const& filename ) const
{
  vcl_ofstream fstr( filename.c_str() );
  ...
  vcl_map< pipeline_node*, unsigned > execution_order_number;
  ...
  // later.
  vcl_string port_node_style = "shape=none,height=0,width=0,fontsize=7";
  vcl_string port_edge_style = "color=black";
  for( node_itr nit = insert_order_.begin();
       nit != insert_order_.end(); ++nit )
  {
    pipeline_node* node = *nit;
    ...
    else
    {
      ...
      vcl_set< vcl_string > const& inp_set = inp_ports[ node->name() ];
      vcl_set< vcl_string > const& out_set = out_ports[ node->name() ];

      typedef vcl_set< vcl_string >::const_iterator set_iter_type;

      for( set_iter_type it = inp_set.begin(); it != inp_set.end(); ++it )
      {
        char optional = ( (*it)[0] == 'O' );
        vcl_string port_name = it->substr(1) + "|" + node->name();
      ...
      {
        vcl_string port_name = *it + "|" + node->name();
       ...
       nit != insert_order_.end(); ++nit )
  {
    ...
    vcl_string new_file = filename.substr( 0,( filename.size( )-4 ) )
      + "_" + node->name() + ".dot";
...
void
pipeline
::output_graphviz_legend( vcl_ostream& fstr ) const
{
  fstr <<
...
::reorder()
{
  ...
  typedef vcl_map<void*, pipeline_node*>::iterator itr;
...
::visit_depth_first( pipeline_node* node )
{
  ...
  if( color_[ node->id() ] == 1 )
  {
    ...
    log_fatal_error( "Cycle detected in execution dependencies involving node \""
                     << node->name()
...
vcl_vector< pipeline_node* >
pipeline
::downstream_nodes( pipeline_node* n )
{
  // find all downstream nodes
  vcl_vector< pipeline_node* > downstream;
  vcl_deque< pipeline_node* > to_explore;
  to_explore.push_back( n );
  while( !to_explore.empty() )
  {
    n = to_explore.front();
    to_explore.pop_front();
    if( vcl_find(downstream.begin(), downstream.end(), n) == downstream.end() )
    {
...
pipeline_node*
pipeline
::get_node_with_id( void* id ) const
{
  vcl_map<void*, pipeline_node*>::const_iterator it = nodes_.find( id );
  if( it == nodes_.end() ) {
    throw vcl_runtime_error( "couldn't find that node" );
  } else {
    return it->second;
  }
}
...
::output_timing_measurements() const
{
  ...
  vcl_cout << "<DartMeasurement name=\"complete_pipeline\""
           << " type=\"numeric/double\">" << sps
           << "</DartMeasurement>" << vcl_endl;
  vcl_map<vcl_string, double> timing_map = this->collect_node_timing();
  typedef vcl_map<vcl_string, double>::const_iterator itr;
  for( itr it = timing_map.begin(); it != timing_map.end(); ++it )
  {
    vcl_cout << "<DartMeasurement name=\""<< it->first <<"\""
...
void
pipeline
::output_timing_measurements(vcl_ostream& str) const
{
  double sps = double(total_step_count_) * 1000.0 / total_elapsed_ms_;

  str << "Timing table begin" << vcl_endl
      << "   total step count: " << total_step_count_ << vcl_endl
      << "   total elapsed time (sec): " << (total_elapsed_ms_ / 1000.0) << vcl_endl
...
vcl_map<vcl_string, double>
pipeline
::collect_node_timing() const
{
  vcl_map<vcl_string, double> timing_map;
  typedef vcl_vector<pipeline_node*>::const_iterator itr;
  for( itr it = execution_order_.begin(); it != execution_order_.end(); ++it )
  {
    ...
    if (super_process * s = dynamic_cast<super_process*>(p))
    {
      ...
      vcl_map<vcl_string, double> tm = s->get_pipeline()->collect_node_timing();

      // Combine sub-pipeline timing info. Add prefix to name to
      // indicate superprocess.
      typedef vcl_map<vcl_string, double>::const_iterator mitr;
      vcl_string prefix = (*it)->name() + ':';
...
vcl_map< vcl_string, pipeline_node* >
pipeline
::enumerate_nodes() const
{
  typedef vcl_map<void*,pipeline_node*>::const_iterator node_itr;
  vcl_map< vcl_string, pipeline_node* > ret;
