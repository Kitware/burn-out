...
public:
  ...
  ///
  /// On return confidence contains mahalonbis distance to closest mixture component
  bool update( const float_type* value, float& confidence);
  ...
  void debug_dump( std::ostream& ostr ) const;

protected:
  /// First, find out into which mixture component the new data fits,
  /// if any.  Return N if there is no fit.
  // On return confidence contains distance to closest component
  unsigned int best_fit_component(const float_type* value, float& confidence);

  /// Update the weight, mean, and variance of component \a idx.
  void update_component( unsigned idx, const float_type* value );
  ...
  void copy_to_component( unsigned idx, const float_type* value );

  /// The index refers to the sorted components, so that index 0
  /// refers to the component with the highest score.
  inline float_type euclidean_dist_sqr( unsigned idx, const float_type* value ) const;

  /// The index refers to the sorted components, so that index 0
  /// refers to the component with the highest score.
  inline float_type mahanalobis_dist_sqr( unsigned idx, const float_type* value ) const;

  /// The index refers to the sorted components, so that index 0
  /// refers to the component with the highest score.
  inline float_type likelihood( unsigned idx, const float_type* value ) const;
...
protected:
  ...
  std::vector<float_type> mean_data_blk_;
