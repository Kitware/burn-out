...
#ifndef vidtk_windowed_mean_image_process_h_
#define vidtk_windowed_mean_image_process_h_
...
public:
  ...
  vil_image_view<double> const& mean_image() const;

  VIDTK_OUTPUT_PORT( vil_image_view<mean_pixel_type> const&, mean_image );

  /// An image the same size as the source image, with each pixel
  /// representing the local variance.
  vil_image_view<double> const& variance_image() const;

  VIDTK_OUTPUT_PORT( vil_image_view<mean_pixel_type> const&, variance_image );
