...
#define RUN_TEST()                                      \
  ...
  TEST( "Value of step 1", collect->values_[0], 4 );    \
  TEST( "Value of step 2", collect->values_[1], 16 );   \
  TEST( "Value of step 3", collect->values_[2], 36 );   \
  TEST( "Value of step 4", collect->values_[3], 64 );   \
  TEST( "Value of step 5", collect->values_[4], 100 );  \
  TEST( "Value of step 6", collect->values_[5], 144 );  \
  TEST( "Value of step 7", collect->values_[6], 196 );
...
test_sync_in_sync()
{
  ...
  process_smart_pointer< product_of_increments_count< sync_pipeline > > prod( new product_of_increments_count< sync_pipeline >( "sync" ) );
...
test_async_in_sync()
{
  sync_pipeline p;
  process_smart_pointer< numbers > nums( new numbers( "numbers", 14 ) );
  process_smart_pointer< product_of_increments_count< async_pipeline > > prod( new product_of_increments_count< async_pipeline >( "async" ) );
  process_smart_pointer< collect_value > collect( new collect_value( "collect" ) );
...
int test_nested_skip_pipeline( int /*argc*/, char* /*argv*/[] )
{
  testlib_test_start( "test_nested_skip_pipeline" );
