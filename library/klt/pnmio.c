  ...
  int *maxval)
{
  ...
  *nrows = atoi(line);
  ...
  _getNextString(fp, line);
  *maxval = atoi(line);
  if (fread(line, 1, 1, fp) < 1) /* Read newline which follows maxval */
    KLTWarning("(pnmReadHeader) No newline after maxval");
  ...
  int *ncols, int *nrows)
{
  ...
  /* Read binary image data */
  {
    ...
    for (i = 0 ; i < *nrows ; i++)  {
      if (fread(tmpptr, *ncols, 1, fp) < 1)
        KLTError("(pgmRead)  Could not read enough bytes");
      tmpptr += *ncols;
    }
  }
  ...
  int nrows)
{
  ...
  /* Write binary data */
  for (i = 0 ; i < nrows ; i++)  {
    if (fwrite(img, ncols, 1, fp) < 1)
      KLTError( "(pgmWrite)  Couldn't write binary data" );
    img += ncols;
  }
}
  ...
  int nrows)
{
  ...
  for (j = 0 ; j < nrows ; j++)  {
    for (i = 0 ; i < ncols ; i++)  {
      if (fwrite(redimg, 1, 1, fp) < 1 ||
          fwrite(greenimg, 1, 1, fp) < 1 ||
          fwrite(blueimg, 1, 1, fp) < 1)
        KLTError("(ppmWrite)  Couldn't write bytes");
