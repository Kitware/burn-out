...
#define VIDTK_DEFAULT_LOGGER __vidtk_logger_auto_sg_gm_pixel_model_cxx__
VIDTK_LOGGER("sg_gm_pixel_model_cxx");
...
bool
sg_gm_pixel_model
::update( const float_type* value, float &confidence )
{
  ...
  // if any.
  unsigned int best_idx = best_fit_component(value, confidence);
...
void
sg_gm_pixel_model
::debug_dump( std::ostream& ostr ) const
{
//   unsigned const M = param_->num_pix_comp_;
//   unsigned const N = param_->num_mixture_comp_;
//
...
//   }
  ...
  ostr << "     sample count: " << sample_count_ << "  count zero mixures: "
       << count_zero_mixures_ << " slack weight: " << slack_weight_ << std::endl;
  ...
  for( unsigned j = 0; j < N; ++j )
  {
    ...
    ostr << " Variance: " << *this->variance_ptr_[j] << std::endl;
    ostr << std::endl;
  }
}
...
unsigned int
sg_gm_pixel_model
::best_fit_component( const float_type* value, float &confidence)
{
  // Number of mixtures components
  unsigned const N = param_->num_mixture_comp_;
...
void
sg_gm_pixel_model
::update_component( unsigned best_idx, const float_type* value )
{
  // Number of mixtures components
  unsigned const N = param_->num_mixture_comp_;
  ...
  else
  {
    ...
    if( rho_mean == 0.0f )
    {
//       LOG_INFO( alpha_mean << "\t" << likelihood( best_idx, value ) );
      rho_mean = alpha_mean * likelihood( best_idx, value );
    }
    else if( rho_mean < 0.0f )
    {
...
::sort_components( unsigned& best_idx_param )
{
  ...
  #define SWAP( X, Y )                                                  \
    std::swap( score_[X], score_[Y] );                                   \
    std::swap( weight_[X], weight_[Y] );                                 \
    std::swap( variance_ptr_[X], variance_ptr_[Y] );                     \
    std::swap( mean_ptr_[X], mean_ptr_[Y] );
...
void
sg_gm_pixel_model
::copy_to_component( unsigned idx, const float_type* value )
{
  unsigned const M = param_->num_pix_comp_;
  unsigned const N = param_->num_mixture_comp_;
...
sg_gm_pixel_model
::euclidean_dist_sqr( unsigned idx, const float_type* value ) const
{
  // Number of components per pixel
  unsigned const M = param_->num_pix_comp_;
...
sg_gm_pixel_model::float_type
sg_gm_pixel_model
::mahanalobis_dist_sqr( unsigned idx, const float_type* value ) const
{
...
sg_gm_pixel_model::float_type
sg_gm_pixel_model
::likelihood( unsigned idx, const float_type* value ) const
{
  ...
  return std::exp( -float_type(0.5)*dist ) / std::pow( two_pi * var,  M/float_type(2.0) );
}
